...
上图展示成长页、已有棋手对局录入和棋手管理三个核心场景。它是根据当前 0.3.0 页面结构制作的界面预览，具体名称、分数和对局数量会随本机数据变化。
...
- **历史管理**：按结果、段位和日期筛选；新格式对局支持更正、删除和撤销，后续分数自动重算。
- **本地备份与恢复**：导出包含全部棋手和对局的 .goelo.json 文件，恢复前提供预览。
...
临时对手的段位基准分为 1 段 1000 分，每升一段增加 200 分：
...
新录入的临时对手使用 `elo-v2` 规则（系数 100）；历史 `elo-v1` 记录仍按原系数 400 重放，避免升级后悄悄改变历史数据。全胜和全败仍分别使用基准分 ±600。
...
- 分析：区间胜率、净变化、对手段位分布、月度趋势和连胜。
- 历史：完整对局列表、筛选、编辑和删除。
- 我的：棋手档案、初始分、目标、备份与恢复。

## 安装

当前版本：**0.3.0**（versionCode 3）

直接下载仓库中的调试安装包：

[下载 GO ELO Android 0.3.0 APK](dist/go-elo-debug.apk)

如果已经安装过 0.2.0，请直接覆盖安装，不要先卸载，以保留本机数据。应用使用相同的 applicationId 和签名，Room 数据库会自动从旧版本迁移到新版本。

## 数据与隐私

- 所有数据默认保存在设备本地。
- 不需要登录，不主动联网，不上传对局数据。
- 备份文件是可读 JSON，内容包含棋手档案、对局、计算结果和规则版本。
