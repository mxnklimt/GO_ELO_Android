...
@Serializable data class Match(val id: String, val order: Long, val kind: MatchKind, val playedAtEpochMs: Long?, val playedZoneId: String?, val outcome: Outcome, val input: RecordInput?, val opponentElo: Double?, val eloBefore: Double, val delta: Double, val eloAfter: Double, val ruleVersion: String, val legacy: LegacyOrigin? = null)
@Serializable data class AppState(val profile: Profile?, val matches: List<Match>)
