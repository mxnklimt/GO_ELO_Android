...
class HistoryFilterTest {
    private fun native(id: String, order: Long, rank: Int, date: String? = "2026-09-17", zone: String = "UTC") = Match(
        id, order, MatchKind.NATIVE,
        date?.let { java.time.LocalDate.parse(it).atStartOfDay(ZoneId.of(zone)).toInstant().toEpochMilli() },
        date?.let { zone }, Outcome.WIN, RecordInput(rank, 0, 0), 2000.0, 2000.0, 0.0, 2000.0, "elo-v1",
    )
