...
class GoalPolicyTest {
    @Test fun boundariesAndCustomGoal() {
        ...
        assertEquals(1100.0, goalView(Profile(name="x", initialElo=999.0), 999.0).target!!, 0.0)
        assertEquals("9 段参考", goalView(Profile(name="x", initialElo=2700.0), 2700.0).rankLabel)
        assertEquals("9 段基准以上", goalView(Profile(name="x", initialElo=2800.0), 2800.0).rankLabel)
